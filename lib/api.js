...
// warnings }. onProgress(pct|null, msg) receives live progress. fields
// carries optional extra form values — e.g. { instructions } with the
// user's free-text "anything specific you want to redact".
export async function analyze(file, onProgress, fields = {}) {
  const acc = await requestJSON("/api/jobs", { method: "POST", body: multipartBody(file, fields) });
...
// applyToBlob sends the (possibly edited) entity list and returns the
// anonymized document as { blob, name } — no DOM involved, so it works
// from the background service worker (the intercept flow re-attaches the
// file to the page instead of downloading it).
export async function applyToBlob(jobId, mode, entities, fallbackName) {
...
export async function applyAndDownload(jobId, mode, entities, fallbackName) {
  const { blob, name } = await applyToBlob(jobId, mode, entities, fallbackName);
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url;
  a.download = name;
